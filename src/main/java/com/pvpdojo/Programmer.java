...
import com.pvpdojo.character.datatypes.SpotanimData;
import com.pvpdojo.orientation.Orientation;
import com.pvpdojo.orientation.OrientationAction;
import com.pvpdojo.orientation.OrientationInstruction;
import com.pvpdojo.timesheet.keyframe.KeyFrame;
import com.pvpdojo.timesheet.keyframe.KeyFrameType;
import com.pvpdojo.timesheet.keyframe.MovementKeyFrame;
import com.pvpdojo.timesheet.keyframe.OrientationKeyFrame;
...
public class Programmer
{
    ...
    @Inject
    public Programmer(Client client, ClientThread clientThread, PVPDojoPlugin plugin, DataFinder dataFinder)
    {
        this.client = client;
    ...
    private void updateCharacter3D()
    {
        ...
        double currentClientTick = (plugin.getCurrentTick() * Constants.GAME_TICK_LENGTH / Constants.CLIENT_TICK_LENGTH) + clientTickAtLastProgramTick;


            Character character = plugin.dummy.dummyCharacter;
...
/**
    ...
    public void transform3D(WorldView worldView, Character character, double currentClientTick)
    {
        if (!playing)
        {
            KeyFrame mkf = character.findNextKeyFrame(KeyFrameType.MOVEMENT, plugin.getCurrentTime());
...
/**
    ...
    private void setOrientationStatic(Character character, int orientation)
    {
        KeyFrame kf = character.getCurrentKeyFrame(KeyFrameType.ORIENTATION);
        if (kf == null)
        {
        ...
        OrientationKeyFrame keyFrame = (OrientationKeyFrame) kf;
        orientation = getOrientationStatic(keyFrame);
        character.setOrientation(orientation);
    }
...
/**
    ...
    {
        double ticksPassed = plugin.getCurrentTime() - keyFrame.getTick();
