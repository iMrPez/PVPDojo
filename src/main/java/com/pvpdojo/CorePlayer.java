...
public class CorePlayer extends Combatant
{
    ...
    {
        var currentWeaponID = PVPUtility.getWeaponID(client);
    ...
    public void displayDamage(DamageData data)
    {
        ...
        if (data.isSpec)
        {
            for (int i = 0; i < data.getSpecHitCount(); i++)
            {
                ...
                plugin.hitsplatOverlay.HitPlayer(plugin.getTicks(), PVPUtility.getHitsplatSprite((int)damage), (int)damage);
            }
        }
        else
        {
    ...
    public boolean isInRange()
    {
        ...
        return PVPUtility.isInRange(dummyWP, client.getLocalPlayer().getWorldLocation(), spell == null && isMelee ? 1 : 10);
    }
