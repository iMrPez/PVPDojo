...
public class Dummy extends Combatant
{
    ...
    {
        var targetTile = PVPUtility.getRandomWalkableWorldPoint(client, startLocation, 4);
        if (targetTile != null)
        {
    ...
    protected void displayDamage(DamageData data)
    {
        if (data.isSpec)
        {
            for (int i = 0; i < data.getSpecHitCount(); i++)
            {
                ...
                plugin.hitsplatOverlay.HitDummy(plugin.getTicks(), PVPUtility.getHitsplatSprite((int)damage), (int)damage);
            }
        }
        else
        {
        ...
        PVPUtility.playCombatStyleHitSound(client, data.combatStyle);
    ...
    {
        var worldPoint = WorldPoint.fromLocal(client, dummyCharacter.getCharacterObject().getLocation());
        var targetPoint = PVPUtility.getNearestAdjacentTile(worldPoint, client.getLocalPlayer().getWorldLocation(), 0);
